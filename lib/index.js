...
module.exports = class Lyricist {
  ...
  */
  ...
  async _request(path) {
    const url = `https://api.genius.com/${encodeURI(path)}`;
    const headers = {
  ...
  */
  ...
  async _scrapeLyrics(url) {
    ...
    if (!lyrics) {
      // workaround for when Genius serves another page for whatever reason
      const lyric_container = $('div').filter(function () {
        if (!this.attribs.class) return false;
        const classes = this.attribs.class.split(' ');
        return classes.find((e) => e.startsWith('Lyrics__Root-sc'));
      });
      if (!lyric_container[0]) return false;
      $(lyric_container[0]).find('br').replaceWith('\n');
      lyrics = $(lyric_container[0]).text().trim();

      // TODO: find and remove the share buttons container instead of doing this
