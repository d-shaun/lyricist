  ...
  */
  ...
  async _scrapeLyrics(url) {
    ...
    if (!lyrics) {
      ...
      const lyrics_container = $('[class^=Lyrics__Container]');

      try {
        $(lyrics_container)
          .find('[class^=PrimisPlayer__Container]')
          .remove(); // remove that one nested div

        $(lyrics_container)
          .find('div')
          .after('\n'); // add line break after the parent div

        $(lyrics_container)
          .find('i')
          .before('__')
          .after('__'); // <i>italic text</i> to __italic text__
      } catch (e) {
        console.log(e);
        // fix some line-breaks and formatting, nbd if it fails. whatever.
      }
      $(lyrics_container)
        .find('br')
        .replaceWith('\n');
