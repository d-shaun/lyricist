...
module.exports = class Lyricist {
  ...
  */
  ...
  async _scrapeLyrics(url) {
    ...
    // Genius uses #lyrics-root for the modern layout
    const lyricsRoot = $('#lyrics-root');
    if (lyricsRoot.length > 0) {
      lyricsRoot
        .find(
          '[data-lyrics-container="true"], div[class^="Lyrics__Container"], div[class^="Lyrics-sc"]',
        )
        .each((_, element) => {
          const $el = $(element);
          ...
          $el
            .find('i')
            .before('_')
            .after('_'); // <i>italic text</i> to _italic text_
          $el.find('br').replaceWith('\n');
          lyrics_texts.push($el.text().trim());
        });
    } else {
      // Fallback for older layouts which used .lyrics
      const oldLyrics = $('.lyrics');
      if (oldLyrics.length > 0) {
        lyrics_texts.push(oldLyrics.text().trim());
      }
    }

    if (!lyrics_texts.length) return null;

    return lyrics_texts.join('\n').trim();
  }
